...
export class Assignment3 extends Scene {
  ...
  display(context, program_state) {
    ...
    program_state.lights = [
      new Light(vec4(0, 0, 0, 1), color(1, 1, 1, 1), 10 ** sunSize),
    ];

    let model_transform = Mat4.identity();

    // sun
    let sun_transform = model_transform.times(
      Mat4.scale(sunSize, sunSize, sunSize)
    );
    ...
    var planet2_material =
      Math.floor(t) % 2 == 1 ? this.materials.planet2g : this.materials.planet2;
    this.shapes.planet2.draw(
      context,
      program_state,
      planet2_transform,
      planet2_material
    ...
    let ring_transform = planet3_transform.times(Mat4.scale(4, 4, 1));
    ...
    this.shapes.torus2.draw(
      context,
      program_state,
      ring_transform,
      this.materials.planet3r
    );
    ...
    let moon_transform = planet4_transform
      ...
      .times(Mat4.rotation(t, 0, 1, 0));
    this.shapes.planet4.draw(
      context,
      program_state,
      planet4_transform,
      this.materials.planet4
    );
    this.shapes.moon.draw(
      context,
      program_state,
      moon_transform,
      this.materials.moon
    ...
    if (this.attached) {
      const desired = Mat4.inverse(
        this.attached().times(Mat4.translation(0, 0, 5))
      );
      const blend_factor = 0.97;
      const current = desired.map((x, i) =>
        Vector.from(program_state.camera_transform[i]).mix(x, blend_factor)
      );
      var state =
        this.attached() != this.initial_camera_location
          ? current
